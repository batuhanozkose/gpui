...
fn is_emoji_presentation(character: char) -> bool {
    ...
    EMOJI_PRESENTATION_REGEX.is_match(character.encode_utf8(&mut buffer))
}

fn font_has_char(db: &usvg::fontdb::Database, id: usvg::fontdb::ID, character: char) -> bool {
...
fn select_emoji_font(
    ...
    for family_name in families {
        ...
        if fonts.contains(&id) || !font_has_char(db, id, character) {
            continue;
        }
    ...
    pub fn new(asset_source: Arc<dyn AssetSource>) -> Self {
        ...
        let fallback_selection = Box::new(
            move |character: char,
                ...
                if is_emoji_presentation(character)
                    && let Some(id) =
                        select_emoji_font(character, fonts, db.as_ref(), EMOJI_FONT_FAMILIES)
                {
                    return Some(id);
                }

                default_fallback_selection(character, fonts, db)
            },
        );
...
mod tests {
    ...
    fn test_is_emoji_presentation() {
        ...
        for (value, expected) in cases {
            assert_eq!(
                is_emoji_presentation(value.chars().next().unwrap()),
                expected,
                "for char {:?}",
                value
            );
